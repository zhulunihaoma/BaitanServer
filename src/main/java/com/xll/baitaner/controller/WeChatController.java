...
public class WeChatController {
    ...
    public void WeChatServicePublic(HttpServletRequest request, HttpServletResponse response){
        try {
            ...
            if(isGet){ //响应服务器接入
                ...
                //处理消息
                String resp = wxMessageService.wxPublicMessageManager(req);
                out.write(resp);
            }
     ...
     * @param response
     */
    @RequestMapping("wechatservice_applet")
    public void WeChatServiceApplet(HttpServletRequest request, HttpServletResponse response){
        try {
            request.setCharacterEncoding(UTF8);
            response.setCharacterEncoding(UTF8);
            ...
            if(isGet){ //响应服务器接入
                ...
                LogUtils.info(TAG,"小程序 Connect WeChatService---signature: " + signature +
                        "\ntimestamp: " + timestamp + "\nnonce: " + nonce + "\nechostr: " + echostr);

                boolean checkRes = WeChatCheckoutUtils.checkSignature(token, timestamp, nonce, signature);
                LogUtils.info(TAG, "WeChatApplet Check Signature---" + checkRes);
                if(checkRes){
                    out.write(echostr);
                    LogUtils.info(TAG, "小程序 微信服务器接入成功！");
                }
            }else { //响应消息推送
                InputStream inputStream = request.getInputStream();
                BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(inputStream, UTF8));
                ...
                //处理消息
                String resp = wxMessageService.wxAppletMessageManager(req);
                out.write(resp);
            }
